      ...
      <div className="min-w-0 space-y-5 sm:space-y-6">
        <section className="fade-up relative overflow-hidden rounded-[32px] bg-[#102c23] p-5 text-white shadow-[0_28px_70px_rgba(16,44,35,0.18)] sm:p-7 lg:p-8">
          ...
          <div className="relative grid gap-7 lg:grid-cols-[1fr_0.7fr] lg:items-center">
            <div className="min-w-0">
              ...
              <h1 className="mt-5 max-w-3xl text-3xl font-semibold leading-tight tracking-[-0.045em] sm:text-4xl">{copy.title}</h1>
        ...
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          {roleStats.map(({ label, value, icon: Icon, accent, surface, note }, index) => (
            <article key={label} className="soft-card lift-card relative min-w-0 overflow-hidden rounded-[26px] p-4 sm:p-5" style={{ animationDelay: `${index * 70}ms` }}><div className={`absolute inset-x-0 top-0 h-1 bg-gradient-to-r ${accent}`} /><div className="flex items-start justify-between gap-4"><div className="min-w-0"><p className="break-words text-xs font-semibold text-slate-500">{label}</p><p className="mt-2 break-words text-3xl font-semibold tracking-[-0.05em] text-slate-950">{value}</p></div><span className={`grid h-11 w-11 shrink-0 place-items-center rounded-2xl ${surface}`}><Icon className="h-5 w-5" /></span></div><p className="mt-4 break-words text-[11px] leading-4 text-slate-400">{note}</p></article>
          ))}
        </div>
